...
module.exports = function(json, extend) {
    ...
    var posts = json.feed.entry ? json.feed.entry.map(function(post, i) {
        ...
        // replace image paths
        markdown = markdown.replace(/\!\[\]\((\bhttps?:\/\/[^\s()<>]+(?:\([\w\d]+\)|[^[:punct:]\s]|\/))/ig, '![](/content/images/blogger/');

        // normailse filenames
        markdown = markdown.replace(/\!\[\]\((.*?)\)/g, function(str, match) {
            return '![](' + match.replace(/[^a-z0-9-_\.\/]/gi, '_').toLowerCase().replace('_apos', '_') + ')';
        });

        var authorName = post.author[0].name['$t'],
            authorId = json.authors[authorName] !== undefined ? json.authors[authorName].id : 1;
    ...
    var users = [];
    Object.keys(json.authors).forEach(function(author){
        users.push({
            name: author,
            id: json.authors[author].id,
            email: json.authors[author].email
        });
    });
